...
export default {
  ...
  watch: {
    $route() {
      ...
      if (
        (useWindowScroll().x < 991 && this.layout.settings.layout === "LosAngeles") ||
        (useWindowScroll().x < 991 && this.layout.settings.layout === "Singapore") ||
        (useWindowScroll().x < 991 && this.layout.settings.layout === "Barcelona")
      ) {
        const newlayout = JSON.parse(
          JSON.stringify(this.layoutobj).replace("horizontal-wrapper", "compact-wrapper")
        );
